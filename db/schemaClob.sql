...
create index it1 on t1(f3) indextype is PC.Solr 
  parameters('{CommitOnSync:true,DefaultColumn:text,IncludeMasterColumn:false,ExtraCols:"f3 \"id\", f2 \"text\""}')
;

alter index it1 rebuild
  parameters('{ExtraCols:"f3 \"id\",f2 \"text\",t2.f5 \"features\"",ExtraTabs:t2,WhereCondition:"L$MT.f3=t2.f4"}')
;
...
BEGIN
    ...
    SolrPushConnector.enqueueChange(USER||'.IT1',ridlist,'update');
END
;
